...
@app.get('/api/{user}')
async def get_person(db: db_dependency, user: int | str = Path(title="The ID or name of the user")):
    crud.get_person(db=db, user=user)

@app.patch("/api/{user}")
async def update_person(updated_person: schemas.PersonBase,   db: db_dependency, user:int |str = Path(title="The ID or name of the user")):
    crud.update_person(updated_person=updated_person, db=db, user=user)

@app.delete("/api/{user}")
async def delete_person(db:db_dependency, user:int |str = Path(title="The ID or name of the user")):
